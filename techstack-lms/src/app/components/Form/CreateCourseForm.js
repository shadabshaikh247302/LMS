...
export default function CreateCourseForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    let newCourse;
    if (!adminState.token) {
      newCourse = await createCourse(state, formData);
    } else {
      newCourse = await createCourse(adminState, formData);
    }
    if (newCourse) {
  ...
  // Handle employee selection change (from EmployeeDropdown)
  const handleTrainerSelectionChange = (selectedEmployees) => {
    // Update the "Trainer" field with the selected employee IDs
    setFormData((prevData) => ({
      ...prevData,
      Trainers: selectedEmployees
    }));
  };
